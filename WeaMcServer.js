...
const server = http.createServer((req, res) => {
  ...
  else if (req.method.toLowerCase() === 'post' && path === 'processchunk') {
    let form = new formidable.IncomingForm();
    form.parse(req, function (err, fields, files) {
      if (err) {
        console.log(`Form error: ${err.message}`);
        ...
        con.query(`SELECT intercoords, blockid FROM chunks WHERE wcoords = '${fields.x},${fields.y}'`, function (err, recordset) { 
          ...
          if (isEmptyObject(recordset)) {
            ...
            chunks = chunk0.concat(chunk1, chunk2, chunk3, chunk4, chunk5, chunk6, chunk7, chunk8, chunk9, chunk10, chunk11, chunk12, chunk13, chunk14, chunk15);
            //console.log(chunks);
            //console.log(chunk0);
  ...
  else if (path.search(/command\?*/i) >= 0) {
    console.log('command');
